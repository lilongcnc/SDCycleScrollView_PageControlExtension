...
- (void)updateDots
{
    ...
    for (NSInteger i = 0; i < self.numberOfPages; i++) {
        ...
        if (i < self.dots.count) {
            ...
            dot = [self generateDotView:i]; //创建单个pageControl,并且设置大小
        }
        
        [self updateDotFrame:dot atIndex:i]; //给前边创建出来的单个pageControl设置想,y
    }
 ...
 */
- (UIView *)generateDotView:(NSInteger)index
{
    UIView *dotView;
    NSLog(@"%@",self.dotViewClass);
...
//        return;
        ...
        for (int i = 0; i < self.numberOfPages; i++) {
            ...
            int lengthMargin1 = 0;
            if (index != self.numberOfPages-1 && i > index) {
                lengthMargin1 = 12+5;
            }
...
//                lengthMargin2 = (5+5)*(i-1<0?0:i-1);
            ...
            NSLog(@"开始循环:index:%zd i:%d lengthMargin1->%d  lengthMargin2->%d",index,i,lengthMargin1,lengthMargin2);

            tempDotView.sd_x = lengthMargin1+ lengthMargin2;
            
            
            if (i == index) {
                tempDotView.image = self.currentDotImage;
                [UIView animateWithDuration:0.3f animations:^{
                    tempDotView.sd_width = 12;
                }];
                
            }else{
                tempDotView.image = self.dotImage;
                [UIView animateWithDuration:0.3f animations:^{
